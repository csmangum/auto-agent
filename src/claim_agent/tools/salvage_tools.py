...
def record_dmv_salvage_report(
    ...
    Returns:
        JSON with recorded DMV report details on success (including NMVTIS-related
        keys when submission runs). On validation error, missing claim, or other
        errors, JSON with ``error`` (message string) and ``claim_id`` only.
    """
