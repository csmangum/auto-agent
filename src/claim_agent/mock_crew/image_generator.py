...
def generate_damage_image(
    ...
    *,
    ...
    Args:
        ...
        claim_id: Optional claim ID for filename. When seed is set, filename is
            deterministic from claim context hash.
    ...
    try:
        ...
        logger.warning("OpenRouter image generation failed: %s", e.response.text)
        raise ValueError(f"OpenRouter image generation failed: {e.response.status_code}") from e
    ...
    if seed is not None:
        # Deterministic filename for reproducible tests
