...
**represented_by_id**: ID of the attorney party representing this party. Only valid when updating an existing party (attorney must already exist in `claim_parties`). At claim creation, `represented_by_id` is ignored—use `update_claim_party` after creating the claim to link a claimant to their attorney.
